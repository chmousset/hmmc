...
class TestMathFixedPoint(unittest.TestCase):
    def test_conversion_ok(self):
        ...
        converter = FloatFixedConverter(res, res - 1, True, saturate=True)
        converter4 = FloatFixedConverter(res, res - 3, True, saturate=True)
    ...
    def test_conversion_error(self):
        converter = FloatFixedConverter(8, 7, True, saturate=False)
        self.assertRaises(ValueError, converter.convert_float, -2.0)
        self.assertRaises(ValueError, converter.convert_float, 2.0)

        converter = FloatFixedConverter(8, 6, signed=False)
